...
def train():
    ...
    if not FLAGS.seperate_upsampler:
        ...
        trainer = GaussianDiffusionTrainer(
            ...
            FLAGS.num_class, FLAGS.cfg, FLAGS.cb, FLAGS.tau, weight, FLAGS.finetune, FLAGS.temperature_beta, FLAGS.temperature_beta_lambda, FLAGS.edm2_truncate, FLAGS.edm2_truncate_portion).to(device)
        ...
        if FLAGS.ckpt_step != 0 and FLAGS.ckpt_step <= FLAGS.seperate_unconditional_step:
            raise ValueError('ckpt_step should be larger than seperate_unconditional_step') #test
        ...
        with trange(FLAGS.seperate_unconditional_step, FLAGS.seperate_unconditional_step + FLAGS.seperate_upsampler_step, dynamic_ncols=True) as pbar:
            for step in pbar:
                ...
                if FLAGS.save_step > 0 and step % FLAGS.save_step == 0:
                    ...
                    torch.save(ckpt, os.path.join(FLAGS.logdir, 'ckpt_{}_stage2.pt'.format(step)))
...
def eval():
    ...
    ckpt = torch.load(os.path.join(FLAGS.logdir, 'ckpt_{}_stage2.pt'.format(FLAGS.ckpt_step)), map_location='cpu')
